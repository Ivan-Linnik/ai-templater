...
function ensurePathExists(targetPath: string, label: string): void {
    if (!fs.existsSync(targetPath)) {
        throw new Error(`Required source is missing: ${label} (${targetPath})`);
    }
}

function getAgentFiles(direction: TDirection, packageRoot: string): TFileToCopy[] {
...
async function confirmClaudeRewrite(rl: readline.Interface): Promise<boolean> {
    const answer = await rl.question(
        `${pc.bold('.claude already exists in the project root. Do you want to replace it?')} (y/N): `
    );
...
export async function runInit(options: TInitOptions): Promise<void> {
    ...
    try {
        ...
        // ─── Validate critical source files before mutating target ────────────
        ...
        output.info(
            `Initializing Claude setup for project "${projectName}" [${directionLabel}]`
        );
        ...
        if (writeClaudeMdResult === 'skipped') {
            output.info(
                'CLAUDE.md already exists in the project root. Keeping the existing file'
            );
        }
