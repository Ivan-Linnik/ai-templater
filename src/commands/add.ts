...
function parseDirection(value: string): TDirection | undefined {
    const normalized = value.trim().toLowerCase();

    if (['fe', 'frontend'].includes(normalized)) return 'fe';
    if (['be', 'backend'].includes(normalized)) return 'be';

    return undefined;
}

function getDirectionAgentFiles(
...
function getDirectionSharedFiles(
    direction: TDirection,
    packageRoot: string,
    language?: TLanguage
): TFileToCopy[] {
    ...
    const names =
        direction === 'fe' ? feFiles : [...beFiles, ...(language === 'ts' ? beTs : [])];
...
export async function runAdd(options: TAddOptions): Promise<void> {
    ...
    try {
        const direction: TDirection =
            options.directionArg !== undefined
