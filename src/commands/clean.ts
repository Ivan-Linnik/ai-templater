...
async function confirmRemove(rl: readline.Interface, question: string): Promise<boolean> {
...
export async function runClean(options: TCleanOptions): Promise<void> {
    ...
    try {
        if (!isForce) {
            const confirmed = await confirmRemove(
                rl,
                `${pc.bold('Remove .claude/ from this project?')} (y/N): `
            );
        ...
        if (isClaudeMdExists) {
            const shouldRemoveMd =
                isForce ||
                (await confirmRemove(
                    rl,
                    `${pc.bold('CLAUDE.md found in project root. Remove it too?')} (y/N): `
                ));
